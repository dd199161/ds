...
Log.install = function(Vue) {
  Vue.prototype.$styleLog =
    ...
    function(
      value,
      str = 'debug', { style = 'color:fuchsia;font-size:24px', ...props } = {}
    ) {
...
export async function createQR(
  context,
  url,
  prop, { width = 150, height = 150, margin = 1, ...props } = {}
) {
  context.qrImgSrc = await (await
    import ( /* webpackChunkName: "qrcode" */ 'qrcode')).toDataURL(
    (context.randCode = (await context.$axios.$post(url)).data[prop]), { width, height, margin, ...props }
  )
}
