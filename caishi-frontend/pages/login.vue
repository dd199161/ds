...
export default {
  ...
  methods: {
    login() {
      ...
      this.$form.validate(valid => {
        if (valid) {
          this.$axiosPlus(
            '/user/login',
            { ip, ...pickBy((val, key) => val !== '', form), code: form.GAKey },
            data => {
              if (data.user_token) {
                this.done(data)
    ...
    getCaptcha() {
      const { ip } = this
      this.$axiosPlus('/user/get-captcha', { device_type, ip }, data => {
        if (data) {
    ...
    playDemo() {
      this.$axiosPlus('user/demo', { ip: this.ip, device_type }, data => {
        if (data.user_token) {
          this.done(data)
        }
      })
    ...
    toggleStorePw(val) {
      ...
      name &&
        this.setState({
          key: 'passwords',
          value: { ...this.passwords, [`${name}`]: val ? password : false }
        })
    },

    handleForgetPassword() {
      this.$axiosPlus(
        'user-safety-question/get-list-no-login',
        { name: this.form.name },
        data => {
          if (data) {
    ...
    showDialog(view) {
      ...
      this.dialogTitle =
        view === 'ProtectionAnswer' ? '回答密保问题' : '设置新密码'
      this.view = view
      this.visible = true
    },
