...
import { usernameReg, pwReg, getRequiredRule } from '~/plugins/formValidate'
        ...
        name: [
          getRequiredRule('请填写用户名'),
          {
            validator: (rule, value, callback) => {
              ...
              validateMessage !== quotaValidateMessage &&
                this.quotaFields &&
                (validateMessage = quotaValidateMessage) &&
                this.quotaFields.clearValidate()
              callback()
            },
            trigger: 'blur'
          }
          ...
          this.getUser(userModel => {
            if (this.quota) {
              this.$axiosPlus(
                ...
                res => {
                  if (res && res.error) {
                    //res -> data:null plugins/ajax.js:27
