    ...
    <el-form :inline="true" class="table-tool-bar" v-if="!hideForm">
      <el-form-item label="根据：">
        <el-select v-model="queryKey">
          <el-option v-for="(label,key) in queryKeyLabels" :label="label" :value="key + 1" :key="key" />
        </el-select>
      </el-form-item>
    ...
    <el-table :data="tableData" stripe border style="width: 100%">
      ...
      <el-table-column label="操作" v-if="!type">
        <template slot-scope="{row}">
          <el-button v-if="row.cancelable" type="text" size="small" @click="cancel(row)">撤单</el-button>
...
export default {
  ...
  methods: {
    get() {
      ...
      if (
        this.queryKeyLabels[queryKey - 1] === '会员账号' &&
      ...
      if (this.checkDateFormat() === true) {
        ...
        this.$axiosPlus(
          ...
          ({ list, drop_list }) => {
            this.queryKeyLabels = {
              ...queryAll,
              ...drop_list.keyword_type_list
            }
            statusList = this.statusList = {
              ...queryAll,
              ...drop_list.status_list
            }
            this.tableData = list.map(_ => ({
