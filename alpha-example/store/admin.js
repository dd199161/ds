...
export const getters = {
    ...
    unreadCount: state => {
        ...
        return state.inbox ? state.inbox.list.filter(_ => _.read_status === 1 || _.status === 1).length : 0
    }
}
...
export const actions = {
    async getRegister({ state: { register }, commit }, callback = () => {}) {
        ...
        commit('regist', (await this.$axios.$post('user-register-link/get')).data.user_register_links.map(_ => ({..._.content })))
        callback()
    },
    async getBox({ state, commit }, { key = 'inbox', page = 0, limit_type = 1 } = {}) {
        ...
        commit('setState', { key, value: (await this.$axios.$post(`user-message/${key=== 'outbox' ? 'send-message-list' : 'get'}`, { content: 0 })).data })
    }
}
