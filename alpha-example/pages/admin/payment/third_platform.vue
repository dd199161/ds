<template src="../../../templates/admin/payment/third-platform.html"> </template>
...
<script>
  ...
  import { pwReg, getRequiredRule, camelCase } from '~/plugins/common'
  const initTransList = _ => ({ ..._,
  ...
  export default {
    ...
    async asyncData({
      ...
      const {
        data: list
      } = await app.$axios.$post('static-data/third-party-game-platform')
      const availableList = list.filter(_ => _[`${isMobile ? 'mobile' : 'web'}_status`] !== 2 && store.getters._3rdPlatformPrefix.includes(_.prefix))
      const self = [{
        prefix: 'self',
        name: '彩票'
      }]
          ...
          transOut: [
            getRequiredRule('请选择平台', 'change'), {
              validator: (rule, value, callback) => {
                ...
                if(!form.transable && (this.isTransOut ? this.bal : this.availableGames[form.transOut].balance) < form.amount) {
    ...
    methods: {
      ...
      hotAmountChange(amount) {
        ...
        this.form.amount = amount === '全部余额' ? this.isTransOut ? this.bal : form.transOut ? this.availableGames[form.transOut].balance : form.amount : amount
      },
      amountChange(amount) {
      ...
      sign(item, index) {
        ...
        loadings[index] = true
        //Array require rest can be reactive
        this.loadings = [...loadings]
        this.$axiosPlus(`third-party-game/${isSignUp ? 'register' : 'login'}`, {
          platform
        }, _ => {
          ...
          if(isSignUp) {
            ...
            this.setUser({ ...this.userModel,
              third_party_game_platform_prefix: this._3rdPlatformPrefix + `,${platform}`
            })
          }
        })
      },
      sumbit() {
        ...
        this.$form.validate(valid => {
          if(valid) {
            ...
            this.loading = true
            this.$axiosPlus('third-party-game/transfer', {
              type,
              user_password: pw,
              platform,
              money: amount
                ...
                onClose: () => {
                  ...
                  this.setState({
                    ...
                    value: { ...availableGames,
                      [platform]: {
                        balance: +availableGames[platform].balance + +amount * (isTransOut ? 1 : -1)
                      }
                    }
                  })
                }
