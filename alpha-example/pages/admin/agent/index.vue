...
export default {
  ...
  async asyncData({ app, store }) {
    ...
    if (!store.state.user.token) return value
    if (+new Date() > expires) {
  ...
  methods: {
    ...
    get() {
        ...
        this.$axiosPlus(
          ...
          data => {
            ...
            this.chartData.length > 0
              ? import('~/util/echarts').then(({ renderTeamView, getChart }) => {
                  ...
                  this.$nextTick(() => {
                    renderTeamView(this.chartOpt, this.$refs.chart,this.legendSelected)
                    this.spin(false)
                    this.chart = getChart()

                    this.chart && this.chart.on('legendselectchanged',({selected}) => {
                      
                      const result = [...new Set(Object.values(selected))]
                      this.isIndeterminate = result.length !== 1
                      result.length === 1 && (this.legendCheckAll = result[0])
                    })
                  })
                })
              : ((this.chartOpt = []),
      ...
      // if (!this.date) return this.$msgbox('日期范围格式有误！', '错误')
    ...
    legendAllChange(){
      ...
      this.legendSelected = legendSelected.length !== chartOpt.length ? chartOpt.map(({name}) => name) : []

      chartOpt.forEach(({name}) => this.chart.dispatchAction({type:`legend${this.legendSelected.length === chartOpt.length ? '' : 'Un'}Select`,name}))
