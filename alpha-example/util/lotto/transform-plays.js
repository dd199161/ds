...
export const initLottoPlays = (store, lottoName, context, error) => {
  ...
  if(!store.getters['lotto/listMap'].hasOwnProperty(lottoName)) {
    return error ? error({ message: '无此彩种！' }) : context.$router.push('/home')
  }
  return context.$getJson(lottoName)
    .then(({ data }) => {
      cache[lottoName] = data.map(({ c: { i: big_group_identifier
        , a: name, sa: show_name }, s }) => {
        return {
          content: { big_group_identifier, name, show_name },
          small_ways: s.map(({ c: { i: small_group_identifier, a: name, sa: show_name }, b }) => {
            return {
              content: {
              ...
              bets: b.map(({ 
                ...
                a: name, sa: show_name,  c: cal_prize_base, idf: identifier,bp:bet_prize,...props 
...
export const initReposalPlays = (store, lottoName, context, error) => {
  ...
  if(!store.getters['lotto/listMap'].hasOwnProperty(lottoName)) {
    return error ? error({ message: '無此彩種！' }) : context.$router.push('/home')
  }
  return context.$getJson(`${lottoName}.reposal`)
    .then(({ data }) => {
      cache[`${lottoName}.reposal`] = data.map(({s,c:{sa:show_name,i:identifier}}) => {
        return {
          content:{show_name,identifier},
          bets:s.map(({id,idf: identifier,mp:max_bet_prize_group,o:odds,si}) => ({
            // id,
            identifier,
            max_bet_prize_group,
            odds,
