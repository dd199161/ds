...
<script>
    ...
    import {pwReg,getRequiredRule} from '~/plugins/common'
    ...
    methods:{
      submit(){
        this.$refs.form.validate((valid) => {
          if (valid) {
            this.$axiosPlus('user/change-login-password',
              zipObject(['old','new','confirm'].map(_ => `${_}_password`))(Object.values(this.form))
            ,() => {
              this.$message({
                message: '修改成功！',
                type: 'success',
