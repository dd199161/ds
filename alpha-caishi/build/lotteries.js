...
exports.toLottoInfo = (data,typeName) => {
  ...
  const lotto = data.big_ways.map(({content:{identifier:i,name:a,show_name:sa},small_ways}) => ({
    c:{
      // i,
      a,sa},
    s:small_ways.map(({content:{identifier:i,name:a,show_name:sa},bets}) => {
...
exports.toReposalInfo = (data,type) => {
  if(type === 'pk10') {
    let big_ways = [].concat(...data.big_ways.map(_ => _.small_ways))
    ...
    return data.big_ways.map(({content:{show_name:sa,identifier:i},small_ways}) => {
