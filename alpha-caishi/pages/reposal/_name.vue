  ...
  export default {
    ...
    methods: {
      async authResolve() {
        ...
        this.getIssueList()
        this.getBal()
        Object.assign(this, await initReposalPlays(this.$store, this.lottoName, this))
      },
      openHotNums() {
      ...
      getViewOrder(valid = false) {
        ...
        playView.orderType ? playView.orderType.forEach(
          (v, k) => (valid ? !playView[v] : true) && playView.getOrder(k)) : (valid ? !playView[playView.orderStr] : true) && playView.getOrder(valid)
      },
      getView() {
      ...
      getOrder(check) {
        ...
        this.getViewOrder()
        this.playView.order() && (check === false ? this.bet(true) : (this.$order.visible = true))
      },
      ...
      bet(oneKey) {
        ...
        if(this.checkBal($order.sum)) {
          const isOneKey = oneKey === true
          if(!($order.sum > 0) && !isOneKey) return this.checkOrder('請填寫金額！')
          // this._getOrder('valid')
          // this.playView.submit(this.bet)
          this.$axiosPlus('user-bet-lottery/create', {
            ip,
            bets: (isOneKey ? this.order : $order.order).map(_ => {
    ...
    computed: { ...mapGetters({
      ...
      playMaxGroup() {
        return Math.min(...[].concat(...this.playList.map(_ => _.bets.map(_ => _.max_bet_prize_group))))
      },
      reposalType() {
