...
export default {
  ...
  async asyncData({ app, store, params, redirect, error }) {
    ...
    if(types.findIndex(_ => _.type === type) === -1) return error({ message: '无此彩系！' })
    if (types.length == 0) {
      return {
    ...
    const { playList } = await initLottoPlays(
      store,
      //types.slice(1) filters days lotto
      Object.keys(types.slice(1).find(_ => _.type === params.type).infos)[0],
      app,
      error
    )
    return {
